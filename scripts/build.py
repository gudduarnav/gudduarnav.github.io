...
def section_head(c, key, intro=False):
    ...
    {f'<p class="section-intro">{e(c[key+"Intro"])}</p>' if intro else ''}</div>'''
...
<!-- Generated by scripts/build.py. Edit data/content.json or data/publications.json. -->
<html lang="{lang}">
<head>
  <meta charset="utf-8">
...
<div class="page-shell">
  ...
  <main id="main" tabindex="-1">
    ...
    <section class="section" id="publications" aria-labelledby="pub-title">
      ...
      {publications(c, lang)}<p class="record-note">{e(c['pubNote'])}</p>{f'<p class="record-note">{e(c["translationNote"])}</p>' if zh else ''}
    </section>
